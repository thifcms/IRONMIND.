...
// Polyfill Object.hasOwn for older environments (Netlify Functions ja tem
// Node moderno, mas mantido por seguranca -- mesmo polyfill do server.ts)
    ...
    try {
      ...
      if (contentType.includes("text/html")) {
        ...
        if (attempts < maxAttempts) {
          console.warn(`[RETRY ${attempts}/${maxAttempts}] Cold Start detectado. Aguardando 3s...`);
...
// Render (e a maioria dos hosts) terminam o HTTPS num proxy na frente do
  ...
  const loadFbConfig = () => {
    ...
    // Tenta ler o arquivo local primeiro (funciona no Render/dev local).
    // Em ambientes serverless (Netlify), um fs.readFileSync dinâmico pode
    // não ir junto no pacote da função -- por isso sempre cai pra
    // variáveis de ambiente (com os valores conhecidos como padrão) em
    // vez de lançar erro e quebrar o WebAuthn inteiro.
    let fileValues: any = {};
    try {
      const configPath = path.join(process.cwd(), "firebase-applet-config.json");
      fileValues = JSON.parse(require("fs").readFileSync(configPath, "utf8"));
    } catch (e) {
      console.warn("[WebAuthn] firebase-applet-config.json não encontrado (normal em serverless) -- usando variáveis de ambiente.");
    }
    fbConfig = {
      projectId: process.env.FIREBASE_PROJECT_ID || fileValues.projectId || "spherical-leaf-vr5vm",
      apiKey: process.env.FIREBASE_API_KEY || fileValues.apiKey || "",
      databaseId: process.env.FIRESTORE_DATABASE_ID || fileValues.firestoreDatabaseId || "ai-studio-fd99e8db-995e-49af-81da-2345fb4823da",
    };
    if (!fbConfig.apiKey) {
      console.error("[WebAuthn] Nenhuma FIREBASE_API_KEY configurada -- desafios do biométrico vão falhar.");
    }
    return fbConfig;
  };
