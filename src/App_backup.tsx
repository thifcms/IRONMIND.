...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (snap.exists()) {
        ...
        // Ignoring Firestore training/diet as per request, relying only on localStorage
  ...
  useEffect(() => {
    ...
    // Check if either is 90 days old
    const checkRenewal = () => {
      const now = Date.now();
      const limit = 90 * 24 * 60 * 60 * 1000;
      
      if (trainingPlan && now - trainingPlan.createdAt >= limit) {
        setShowRenewalAlert(true);
      } else if (dietPlan && now - dietPlan.createdAt >= limit) {
        setShowRenewalAlert(true);
      }
    };
