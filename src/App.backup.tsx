...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Load plans and history (simplified for now, ideally separate collections or fields)
  ...
  const updateTrainingPlan = async (plan: TrainingPlan) => {
    ...
    const userId = savedUser ? JSON.parse(savedUser).uid : user?.uid;
    if (userId) {
      await updateDoc(doc(db, 'users', userId), { trainingPlan: plan });
    }
  };
  ...
  const updateDietPlan = async (plan: DietPlan) => {
    ...
    const userId = savedUser ? JSON.parse(savedUser).uid : user?.uid;
    if (userId) {
      await updateDoc(doc(db, 'users', userId), { dietPlan: plan });
    }
  };
