...
const USER_ID = "thifcms@gmail.com"; 
 ...
 */
export async function chatWithCoach(history: ChatMessage[], message: string, allowFallback: boolean = false): Promise<ChatMessage> {
  ...
  try {
    ...
    const res = await fetch("/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ messages: stabilized, systemInstruction, allowFallback, userId: USER_ID })
    });
 ...
 */
export async function generateProposal(type: 'training' | 'diet', context: string, allowFallback: boolean = false): Promise<TrainingPlan | DietPlan> {
  const isTraining = type === 'training';
  
  try {
    const schemaRef = isTraining 
      ? `{"name": "Nome do Treino", "days": [{"label": "Dia 1 - Costas", "exercises": [{"name": "Puxada", "sets": 3, "reps": "8-12", "rest": "90s"}]}]}`
    ...
    const res = await fetch("/api/generate-proposal", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt, systemInstruction, allowFallback, userId: USER_ID })
    });
 ...
 */
export async function analyzeFoodImage(imageBase64: string, allowFallback: boolean = false): Promise<{ food: string, calories: number, color: 'green' | 'yellow' | 'red', explanation: string }> {
  try {
    ...
    const res = await fetch("/api/analyze-image", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ imageBase64: cleanBase64, prompt, allowFallback, userId: USER_ID })
    });
